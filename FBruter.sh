...
command -v tor > /dev/null 2>&1 || { echo >&2 "I require tor but it's not installed, run requirements.sh. Aborting."; exit 1; }
...
printf "\n"
printf "\e[1;104m     FBruter v1.0, Coded by: harsh2527      \e[0m\n"
printf "\n"
...
read -p $'\e[1;92mWordlist (Hit Enter to use default list): \e[0m' wl_pass
...
printf "\e[1;92mUse ./FBruter.sh --resume\n"
else
exit 1
fi
...
dat=$(($hourdate + 1))
mindate=$(date +%M:%S)
printf "\e[1;91m[#] Wait 1 hour to return :\e[0m\e[1;93m %s:%s\n\e[0m" $dat $mindate
let turn+=20
sleep 3600
resumemethod2
hourdate2=$(date +%H)
